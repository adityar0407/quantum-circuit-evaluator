...
def _gate_based_model(profile: PhysicalHardwareProfile) -> PhysicalQdkModelMapping:
    ...
    parameters = {
        ...
        "sx_gate_time": _seconds_to_ns_int(_sx_gate_time(profile)),
        "sx_gate_error_rate": _sx_gate_error_rate(profile),
    ...
    model = _build_optional_qdk_model("GateBased", parameters)
    return PhysicalQdkModelMapping(
        ...
        metadata=_metadata(
            ...
            mapping={
                "error_rate": [
                    "max(one_qubit_gate_error_rate, sx_gate_error_rate, two_qubit_gate_error_rate, measurement_error_rate)",
                ],
                "gate_time": ["one_qubit_gate_time seconds -> integer ns"],
                "sx_gate_time": ["sx_gate_time seconds -> integer ns, fallback one_qubit_gate_time"],
                "sx_gate_error_rate": ["sx_gate_error_rate, fallback one_qubit_gate_error_rate"],
            ...
            warnings=[
                "QDK GateBased exposes one aggregate error_rate; separate 1Q, SX, 2Q, and measurement errors are reduced with max(...).",
            ],
        ),
    )
...
def _neutral_atom_model(profile: PhysicalHardwareProfile) -> PhysicalQdkModelMapping:
    ...
    model = _build_optional_qdk_model("NeutralAtom", parameters)
    return PhysicalQdkModelMapping(
        model=model,
        metadata=_metadata(
            profile,
...
def _sx_gate_time(profile: PhysicalHardwareProfile) -> float:
    return profile.sx_gate_time if profile.sx_gate_time is not None else profile.one_qubit_gate_time


def _sx_gate_error_rate(profile: PhysicalHardwareProfile) -> float:
    return profile.sx_gate_error_rate if profile.sx_gate_error_rate is not None else profile.one_qubit_gate_error_rate


def _build_optional_qdk_model(model_name: str, parameters: dict[str, Any]) -> Any:
    try:
        from qdk.qre.models import qubits as qdk_qubits

        model_class = getattr(qdk_qubits, model_name)
